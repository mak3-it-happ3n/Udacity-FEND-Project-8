...
class App extends Component {
  ...
  //send a CORS request with the map object to the foursquare API to find POIs
  // (incl. markers & info windows)
  ...
  loadMarkers(map) {
    ...
    CORSRequest.onload = () => {
      ...
      // clear arrays (markers, info windows)
      markers = [];
      infoWindows = [];
  ...
  //a CORS request function.
  createCORSRequest(method, url) {
    var xhr = new XMLHttpRequest();
    if ("withCredentials" in xhr) {
      // check if "withCredentials" property is present?
      xhr.open(method, url, true);
    } else if (typeof XDomainRequest !== "undefined") {
      // check if XDomainRequest exists
      xhr = new XDomainRequest();
      xhr.open(method, url);

    } else {
      // if CDomainReqzest doesn't exist, CORS is not supported by the browser
      xhr = null;
  ...
  // set the state and roload markers via queryHandler
  queryHandler(query) {
    this.setState({query});
